...
class Mailer extends _base2.default {
  setup() {
    ...
    return _asyncToGenerator(function* () {
      let config = Object.assign(_mailer2.default, _this.config.mailer);

      // create reusable transporter method (opens pool of SMTP connections)
      _this.app.nodemailer = _nodemailer2.default.createTransport(config.transport);

      if (config.plugins) {
        for (let pluginKey of Object.keys(config.plugins)) {
          _this.app.nodemailer.use(pluginKey, config.plugins[pluginKey]);
        }
      }

      yield _this.app.nodemailer.verify();
    })();
  }
}
